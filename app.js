...
const COLS = ['clients','services','quotes','team','assets','suppliers','supplierPayments','payroll','payrollRetentions','purchases','invoices','payments','cashflow','imports','planRequests'];

const INDUSTRIES = {
  hvac:{name:'HVAC',logo:'HV',color:'#0ea5e9',client:'Cliente',clients:'Clientes',service:'Servicio HVAC',services:'Servicios HVAC',team:'Técnicos',payroll:'Nómina técnicos',assets:'Activos',suppliers:'Suplidores HVAC',supplierPayments:'Pagos a suplidores',hero:'Diagnósticos, mantenimientos, garantías, facturas, nómina, suplidores y cobros.',nav:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],serviceFields:['Tipo de servicio','Equipo / Marca','BTU / Modelo','Diagnóstico','Garantía'],assetFields:['Marca','Modelo','BTU','Serial','Ubicación'],supplierFields:['Categoría','Marca principal','Términos','Notas']},
  salon:{name:'Salón / Barbería',logo:'SB',color:'#a855f7',client:'Cliente',clients:'Clientes',service:'Cita',services:'Agenda y Citas',team:'Estilistas',payroll:'Pagos estilistas',assets:'Activos',suppliers:'Suplidores belleza',supplierPayments:'Pagos a suplidores',hero:'Agenda, servicios de belleza, productos, comisiones, cobros y reportes.',nav:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],serviceFields:['Servicio','Profesional','Hora','Duración','Notas de estilo'],assetFields:['Área','Silla / Estación','Estado','Notas'],supplierFields:['Categoría','Producto principal','Términos','Notas']},
  transport:{name:'Transporte',logo:'TR',color:'#2563eb',client:'Cliente',clients:'Clientes',service:'Servicio',services:'Servicios de Transporte',team:'Choferes',payroll:'Pagos a choferes',assets:'Activos',suppliers:'Suplidores / Talleres',supplierPayments:'Pagos a suplidores',hero:'Rutas, millas, facturación, cobros, comisiones, retenciones, flota y suplidores.',nav:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],serviceFields:['Tipo de carga','Evidencia / referencia'],assetFields:['Unidad','Tablilla','VIN','Marbete','Seguro'],supplierFields:['Categoría','Servicio principal','Términos','Notas']},
  handyman:{name:'Handyman',logo:'HM',color:'#f97316',client:'Cliente',clients:'Clientes',service:'Trabajo',services:'Trabajos',team:'Personal',payroll:'Pagos personal',assets:'Activos',suppliers:'Suplidores materiales',supplierPayments:'Pagos a suplidores',hero:'Trabajos livianos, materiales, evidencias, cobros, nómina y suplidores.',nav:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],serviceFields:['Categoría','Área','Materiales','Prioridad','Observaciones'],assetFields:['Herramienta','Estado','Costo','Asignado a'],supplierFields:['Categoría','Material principal','Términos','Notas']},
  cleaning:{name:'Limpieza',logo:'CL',color:'#14b8a6',client:'Cliente',clients:'Clientes',service:'Limpieza',services:'Servicios de Limpieza',team:'Personal',payroll:'Nómina personal',assets:'Activos',suppliers:'Suplidores productos',supplierPayments:'Pagos a suplidores',hero:'Limpiezas residenciales/comerciales, productos, nómina, suplidores, cobros y reportes.',nav:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],serviceFields:['Tipo de limpieza','Área','Frecuencia','Productos','Notas'],assetFields:['Producto / Equipo','Cantidad','Costo','Ubicación'],supplierFields:['Categoría','Producto principal','Términos','Notas']},
  construction:{name:'Construcción',logo:'CO',color:'#64748b',client:'Cliente',clients:'Clientes',service:'Proyecto',services:'Proyectos',team:'Equipo',payroll:'Pagos de obra',assets:'Activos',suppliers:'Suplidores construcción',supplierPayments:'Pagos a suplidores',hero:'Proyectos, etapas, materiales, pagos de obra, suplidores, evidencias y reportes.',nav:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],serviceFields:['Tipo de proyecto','Dirección','Etapa','Materiales','Notas técnicas'],assetFields:['Material / Equipo','Cantidad','Costo','Proveedor'],supplierFields:['Categoría','Material principal','Términos','Notas']}
};
...
const PLANS = {
  free:{name:'Free',price:'$0',badge:'Básico',limits:{clients:5,services:10,quotes:10,team:1,assets:0,suppliers:0,supplierPayments:0,payroll:0,purchases:0,invoices:3,payments:3,cashflow:5},modules:['dashboard','clients','services','quotes','billing','importData','plans','settings'],features:['5 clientes','10 servicios','3 facturas','Sin nómina','Sin suplidores','Sin reportes avanzados']},
  pro:{name:'Pro',price:'$19.99/mes',badge:'Profesional',limits:{clients:500,services:1000,quotes:1000,team:10,assets:100,suppliers:25,supplierPayments:100,payroll:100,purchases:100,invoices:500,payments:500,cashflow:1000},modules:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],features:['Nómina básica','Suplidores','Logo en facturas','Reportes PDF','500 clientes']},
  business:{name:'Business',price:'$39.99/mes',badge:'Premium',limits:{clients:5000,services:10000,quotes:10000,team:50,assets:1000,suppliers:500,supplierPayments:2000,payroll:2000,purchases:2000,invoices:5000,payments:5000,cashflow:10000},modules:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],features:['White-label completo','Nómina avanzada','Control de suplidores','Firma digital','Reportes ejecutivos']},
  enterprise:{name:'Enterprise',price:'Custom',badge:'Corporativo',limits:{clients:Infinity,services:Infinity,quotes:Infinity,team:Infinity,assets:Infinity,suppliers:Infinity,supplierPayments:Infinity,payroll:Infinity,purchases:Infinity,invoices:Infinity,payments:Infinity,cashflow:Infinity},modules:['dashboard','clients','services','quotes','team','payroll','assets','suppliers','supplierPayments','purchases','billing','payments','cashflow','reports','importData','plans','settings'],features:['Ilimitado','Dominio personalizado','Roles futuros','Soporte corporativo']}
};

const TITLES = {dashboard:'Home',clients:'Clientes',services:'Servicios',quotes:'Cotizaciones Pro',team:'Equipo',payroll:'Nómina',assets:'Activos',suppliers:'Suplidores',supplierPayments:'Pagos suplidores',purchases:'Compras',billing:'Facturación',payments:'Cobros',cashflow:'Flujo de caja',reports:'Reportes',importData:'Importar datos',plans:'Planes',settings:'Configuración'};
...
function limits(){['clients','services','quotes','team','assets','suppliers','supplierPayments','payroll','invoices','payments','cashflow'].forEach(c=>{const el=$(c+'Limit');if(el)el.textContent=`Uso: ${limitText(c)}`;});}
...
async function clearDemoData(industryId=''){
  if(!uid()) return;
  const demoCols = ['clients','services','quotes','team','assets','suppliers','supplierPayments','payroll','payrollRetentions','purchases','invoices','payments','cashflow','planRequests'];
...
function importStatsDefault(){return {clients:0,services:0,quotes:0,invoices:0,payments:0,skipped:0,errors:0};}
...
function safeTextFromBuffer(buf){try{return new TextDecoder('utf-8',{fatal:false}).decode(buf);}catch(e){return '';}}
function cleanImportText(txt){return String(txt||'').replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]+/g,' ').replace(/\s+/g,' ').trim();}
function tryJsonParse(txt){try{return JSON.parse(txt);}catch(e){return null;}}
function asArray(v){return Array.isArray(v)?v:(v&&typeof v==='object'?Object.values(v):[]);}
function legacyNumberValue(v){const n=Number(v);return Number.isFinite(n)?n:0;}
function normalizeImportDate(v){const m=String(v||'').match(/20\d{2}-\d{2}-\d{2}/);return m?m[0]:today();}
function normalizeImportStatus(v,type='FAC'){
  const raw=String(v||'').toUpperCase();
  if(raw.includes('PAG')) return type==='FAC'?'Pagada':'Aprobada';
  ...
  return type==='FAC'?'Pendiente':'Borrador';
}
function normalizeMethod(v){const s=String(v||'').toLowerCase();if(s.includes('ath'))return 'ATH Móvil';if(s.includes('cash')||s.includes('efect'))return 'Efectivo';if(s.includes('stripe'))return 'Stripe';if(s.includes('tarjeta'))return 'Tarjeta';if(s.includes('cheque'))return 'Cheque';if(s.includes('transfer'))return 'Transferencia';return v||'Importado';}
function flattenImportDocs(data){
  const out=[];
  const walk=(x)=>{if(!x)return;if(Array.isArray(x))return x.forEach(walk);if(typeof x==='object'){
    if(x.number||x.client||x.items||x.totals||x.type) out.push(x);
    Object.values(x).forEach(v=>{if(v&&typeof v==='object'&&v!==x)walk(v);});
  }};walk(data);return out;
}
function extractLegacyDocsFromText(txt){
  const t=cleanImportText(txt); if(!t) return [];
  const startMatches=[...t.matchAll(/docs"?\s*[^a-zA-Z0-9]{0,8}(doc_\d+_[a-f0-9]+)/gi)];
  const starts=startMatches.length?startMatches.map(m=>m.index):[...t.matchAll(/doc_\d+_[a-f0-9]+/gi)].map(m=>m.index).filter((v,i,a)=>a.indexOf(v)===i);
  const docs=[];
  const val=(chunk,key,max=160)=>{const re=new RegExp('\\b'+key+'\\b[^A-Za-z0-9]{0,16}(.{1,'+max+'}?)(?=\\s+(date|notes|totals|type|createdAt|taxRate|validUntil|client|contact|name|addr|id|items|desc|qty|price|catId|svcId|terms|status|lastPdfUrl|lastPdfSentAt|number|updatedAt|_createdAtServer)\\b|$)','i');const m=chunk.match(re);return m?String(m[1]||'').replace(/^[*#.!+\-\s]+/,'').replace(/z[\w)]?$/,'').trim():'';};
  const dateVal=(chunk,key)=>{const m=chunk.match(new RegExp('\\b'+key+'\\b[^0-9]{0,24}(20\\d{2}-\\d{2}-\\d{2})','i'));return m?m[1]:'';};
  for(let i=0;i<starts.length;i++){
    const chunk=t.slice(starts[i], starts[i+1]||t.length);
    const number=(chunk.match(/\b(?:FAC|COT|Q)[- ]?20\d{2}[- ]?\d{3,6}/i)||[])[0]||'';
    const explicitType=(chunk.match(/\btype\b[^A-Za-z0-9]{0,16}(FAC|COT)/i)||[])[1]||'';
    if(!number && !explicitType) continue;
    const type=(explicitType||(/^COT|^Q/i.test(number)?'COT':'FAC')).toUpperCase();
    const itemMatches=[...chunk.matchAll(/\bdesc\b[^A-Za-z0-9]{0,18}(.{3,160}?)(?=\s+(qty|price|catId|svcId|items|_createdAtServer|terms|status|number)\b)/gi)];
    const items=itemMatches.map(m=>({description:String(m[1]||'').replace(/^[*#.!+\-\s]+/,'').replace(/z[\w)]?$/,'').trim(),qty:1,price:0})).filter(x=>x.description);
    const totalMatch=[...chunk.matchAll(/\bgrand\b[^0-9]{0,24}(\d+(?:\.\d{1,2})?)/gi)].pop();
    const pdf=(chunk.match(/https:\/\/firebasestorage\.googleapis\.com\/[^\s]+/i)||[])[0]||'';
    docs.push({legacyId:(chunk.match(/doc_\d+_[a-f0-9]+/i)||[])[0],type,number:number||'',date:normalizeImportDate(dateVal(chunk,'date')),validUntil:normalizeImportDate(dateVal(chunk,'validUntil')),client:{name:val(chunk,'name',90),contact:val(chunk,'contact',30),addr:val(chunk,'addr',150)},items,notes:val(chunk,'notes',200),terms:val(chunk,'terms',200),status:val(chunk,'status',30),totals:{sub:totalMatch?Number(totalMatch[1]):0,grand:totalMatch?Number(totalMatch[1]):0,tax:0},lastPdfUrl:pdf});
  }
  const seen=new Set();
  return docs.filter(d=>{const k=d.legacyId||d.number; if(!k||seen.has(k))return false; seen.add(k); return true;});
}
function normalizeImportDoc(d){
  const client=d.client||{};
  const type=String(d.type||(/^FAC/i.test(d.number||'')?'FAC':'COT')).toUpperCase().includes('FAC')?'FAC':'COT';
  const items=asArray(d.items).map(it=>({description:it.description||it.desc||it.title||it.name||'Servicio importado',qty:Number(it.qty||it.quantity||1),price:Number(it.price||it.unitPrice||it.amount||0)}));
  const subtotal=Number(d.subtotal ?? d.totals?.sub ?? d.totals?.subtotal ?? items.reduce((a,x)=>a+(Number(x.qty||1)*Number(x.price||0)),0));
  const ivu=Number(d.ivu ?? d.tax ?? d.totals?.tax ?? 0);
  const total=Number(d.total ?? d.grand ?? d.totals?.grand ?? (subtotal+ivu));
  return {legacyId:d.legacyId||d.id||'',type,number:d.number||'',date:normalizeImportDate(d.date||d.createdAt),validUntil:normalizeImportDate(d.validUntil||d.dueDate),clientName:client.name||d.clientName||d.customerName||'Cliente importado',phone:client.contact||client.phone||d.phone||'',email:client.email||d.email||'',address:client.addr||client.address||d.address||'',items,subtotal,ivu,total,notes:d.notes||'',terms:d.terms||'',status:normalizeImportStatus(d.status,type),pdfUrl:d.lastPdfUrl||d.pdfUrl||d.url||'',method:normalizeMethod(d.terms||d.paymentMethod||'')};
}
function findExistingClientByImport(n,phone){const name=String(n||'').toLowerCase().trim();const tel=String(phone||'').replace(/\D/g,'');return state.clients.find(c=>(name&&String(c.name||'').toLowerCase().trim()===name)||(tel&&String(c.phone||'').replace(/\D/g,'')===tel));}
async function importHistoricalDocs(docs){
  const stats=importStatsDefault();
  for(const raw of docs){
    try{
      const d=normalizeImportDoc(raw); if(!d.clientName){stats.skipped++;continue;}
      let c=findExistingClientByImport(d.clientName,d.phone);
      if(!c){const ref=await addDoc(colPath('clients'),{name:d.clientName,phone:d.phone,email:d.email,city:'',address:d.address,tags:'Histórico, Importado',notes:`Importado desde histórico ${d.legacyId||d.number||''}`,createdAt:serverTimestamp(),updatedAt:serverTimestamp()}); c={id:ref.id,name:d.clientName,phone:d.phone,email:d.email,address:d.address}; stats.clients++;}
      const serviceTitle=(d.items[0]?.description||d.notes||'Servicio histórico').slice(0,180);
      const serviceRef=await addDoc(colPath('services'),{clientId:c.id,clientName:c.name,date:d.date,status:d.type==='FAC'?'Facturado':'Pendiente',priority:'Normal',serviceType:'Histórico',title:serviceTitle,amount:d.subtotal||d.total||0,items:d.items,fields:[d.number,d.notes,d.pdfUrl].filter(Boolean),legacyId:d.legacyId,createdAt:serverTimestamp(),updatedAt:serverTimestamp()}); stats.services++;
      if(d.type==='FAC'){
        const invRef=await addDoc(colPath('invoices'),{number:d.number||`FAC-${new Date().getFullYear()}-IMP-${String(Date.now()).slice(-5)}`,clientId:c.id,clientName:c.name,serviceId:serviceRef.id,serviceTitle,issueDate:d.date,dueDate:d.validUntil||d.date,subtotal:d.subtotal,ivu:d.ivu,taxPercent:d.subtotal?((d.ivu/d.subtotal)*100):0,total:d.total||d.subtotal,status:d.status,notes:d.notes,terms:d.terms,pdfUrl:d.pdfUrl,legacyId:d.legacyId,createdAt:serverTimestamp(),updatedAt:serverTimestamp()}); stats.invoices++;
        if(d.status==='Pagada') {await addDoc(colPath('payments'),{invoiceId:invRef.id,invoiceNumber:d.number,date:d.date,method:d.method,amount:d.total||d.subtotal,note:'Pago importado del histórico',createdAt:serverTimestamp(),updatedAt:serverTimestamp()}); stats.payments++;}
      } else {
        await addDoc(colPath('quotes'),{number:d.number||`COT-${new Date().getFullYear()}-IMP-${String(Date.now()).slice(-5)}`,clientId:c.id,clientName:c.name,date:d.date,validUntil:d.validUntil,status:d.status,priority:'Normal',serviceType:'Histórico',title:serviceTitle,items:d.items,subtotal:d.subtotal,ivu:d.ivu,taxPercent:d.subtotal?((d.ivu/d.subtotal)*100):0,total:d.total||d.subtotal,notes:d.notes,terms:d.terms,pdfUrl:d.pdfUrl,legacyId:d.legacyId,createdAt:serverTimestamp(),updatedAt:serverTimestamp()}); stats.quotes++;
      }
    }catch(e){console.error('Import error',e,raw);stats.errors++;}
  }
  await addDoc(colPath('imports'),{date:new Date().toISOString(),source:'Importador histórico',stats,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
  return stats;
}
function renderImportHistory(){const box=$('importHistory'); if(!box)return; const rows=[...(state.imports||[])].slice(-10).reverse(); box.innerHTML=rows.length?table(['Fecha','Clientes','Servicios','Cotizaciones','Facturas','Cobros','Errores'],rows.map(x=>`<tr><td>${esc(String(x.date||'').slice(0,19).replace('T',' '))}</td><td>${x.stats?.clients||0}</td><td>${x.stats?.services||0}</td><td>${x.stats?.quotes||0}</td><td>${x.stats?.invoices||0}</td><td>${x.stats?.payments||0}</td><td>${x.stats?.errors||0}</td></tr>`)):'<p class="muted">Sin importaciones todavía.</p>';}
function renderImportData(){
  const box=$('importSummary'); if(!box)return;
  box.innerHTML=`<div class="import-drop"><h3>Business Data Engine</h3><p>Importa exportaciones de Firebase/Firestore, JSON, CSV simple o el archivo histórico de Oasis. El sistema crea clientes, servicios, facturas, cotizaciones, pagos y KPI automáticamente.</p><input id="legacyImportFile" type="file" accept=".json,.csv,.txt,.bin,.backup,application/json"><div class="form-actions"><button id="runLegacyImport" class="primary" type="button">Importar histórico</button><button id="downloadImportTemplate" type="button">Plantilla JSON</button></div><small class="muted">Gobierno del dato: no borra registros existentes; solo añade y deja auditoría.</small></div>`;
  renderImportHistory();
  const tpl=$('downloadImportTemplate'); if(tpl) tpl.onclick=()=>{const sample=[{type:'FAC',number:'FAC-2026-0001',date:today(),status:'Pagada',client:{name:'Cliente Demo',contact:'787-000-0000',addr:'San Juan PR'},items:[{description:'Mantenimiento preventivo',qty:1,price:75}],totals:{sub:75,tax:0,grand:75},terms:'Pagado por ATH Móvil'}]; const blob=new Blob([JSON.stringify(sample,null,2)],{type:'application/json'}); const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='plantilla-importacion-nexus.json';a.click();URL.revokeObjectURL(a.href);};
  const btn=$('runLegacyImport'); if(btn) btn.onclick=async()=>{const f=$('legacyImportFile')?.files?.[0]; if(!f) return alert('Selecciona el archivo primero.'); btn.disabled=true; btn.textContent='Importando...'; try{const buf=await fileToArrayBuffer(f); const txt=safeTextFromBuffer(buf); let docs=[]; const json=tryJsonParse(txt); if(json) docs=flattenImportDocs(json); else if(f.name.toLowerCase().endsWith('.csv')){docs=txt.split(/\r?\n/).slice(1).map(line=>{const p=line.split(',');return {type:'FAC',date:p[0],client:{name:p[1]},items:[{description:p[2]||'Servicio',qty:1,price:Number(p[3]||0)}],totals:{sub:Number(p[3]||0),grand:Number(p[3]||0)},status:p[4]||'Pendiente'};});} else docs=extractLegacyDocsFromText(txt); if(!docs.length) throw new Error('No se detectaron documentos válidos.'); const stats=await importHistoricalDocs(docs); $('importResult').innerHTML=`<div class="ok-box"><b>Importación completada</b><br>Clientes: ${stats.clients} · Servicios: ${stats.services} · Cotizaciones: ${stats.quotes} · Facturas: ${stats.invoices} · Cobros: ${stats.payments} · Errores: ${stats.errors}</div>`; renderImportHistory(); }catch(e){console.error(e);$('importResult').innerHTML=`<div class="danger-box"><b>No se pudo importar:</b> ${esc(e.message||e)}</div>`;} finally{btn.disabled=false;btn.textContent='Importar histórico';}};
}
...
function previewQuote(id){
  const q=state.quotes.find(x=>x.id===id); if(!q)return;
  const p=profile(), c=clientBy(q.clientId), t=quoteTotals(q), logo=p.logoPdf||p.logoDashboard;
  const rows=quoteItemsFallback(q).map(it=>`<tr><td>${esc(it.description||'Servicio')}</td><td>${esc(it.qty||1)}</td><td>${money(it.price||0)}</td><td>${money(Number(it.qty||1)*Number(it.price||0))}</td></tr>`).join('');
...
function render(){setVisuals();nav();forms();bindServiceItems();bindQuoteItems();bindServiceProductivity();if(isTransport()){['sOrigin','sDestination','sRouteMiles','sRouteRate','sRouteBase'].forEach(id=>$(id)&&($(id).oninput=updateTransportTotal));updateTransportTotal();}kpis();renderHomePolish();tables();renderImportData();plans();renderWelcomeCenter();enforceModuleView();$('pageTitle').textContent=T(TITLES[state.activeView]||state.activeView);$('pageSubtitle').textContent=state.activeView==='dashboard'?T('Resumen operativo, financiero y alertas'):' ';applyLanguage();}
