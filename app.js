...
const COLS = ['clients','services','team','assets','suppliers','supplierPayments','payroll','purchases','invoices','payments','cashflow','planRequests'];
...
const TITLES = {dashboard:'Dashboard',clients:'Clientes',services:'Servicios',team:'Equipo',payroll:'Nómina',assets:'Activos',suppliers:'Suplidores',supplierPayments:'Pagos suplidores',purchases:'Compras',billing:'Facturación',payments:'Cobros',cashflow:'Flujo de caja',reports:'Reportes',plans:'Planes',settings:'Configuración'};
...
function globalSearchResults(q){
  ...
  state.team.forEach(x=>hit(x.name,x.role,x.phone,x.email)&&rows.push({type:'Empleado',title:x.name,meta:[x.role,x.status].filter(Boolean).join(' · '),view:'team'}));
...
async function clearDemoData(industryId=''){
  if(!uid()) return;
  const demoCols = ['clients','services','team','assets','suppliers','supplierPayments','payroll','purchases','invoices','payments','cashflow','planRequests'];
...
function tables(){const i=industry();
  ...
  $('payrollTable').innerHTML=table(['Fecha',i.team,'Periodo','Bruto','Bonos','Descuentos','Neto','Acción'],state.payroll.map(p=>`<tr><td>${esc(p.date)}</td><td>${esc(p.teamName)}</td><td>${esc(p.period)}<br><span class="muted">${Number(p.hours||0)}h + ${Number(p.overtime||0)}h extra</span></td><td>${money(p.gross)}</td><td>${money(p.bonus)}</td><td>${money(Number(p.deductions||0)+Number(p.advance||0))}</td><td><b>${money(p.net)}</b><br><span class="muted">${esc(p.method)}</span></td><td><div class="actions"><button data-paystub="${p.id}" type="button">PDF</button>${action('payroll',p.id)}</div></td></tr>`));
...
async function saveSettings(){const p={...profile()};['businessName','slogan','phone','whatsapp','email','web','address','merchant','representative','tax','transportRatePerMile','transportBaseCharge','primaryColor','secondaryColor','language'].forEach(k=>p[k]=$('set_'+k)?.value||'');p.industry=$('set_industry').value;p.customServices={...(p.customServices||{})};p.customServices[p.industry]=($('set_services')?.value||'').split('\n').map(x=>x.trim()).filter(Boolean);for(const k of ['logoDashboard','logoPdf','favicon','signature']){const v=await fileData($('set_'+k));if(v)p[k]=v;}await setDoc(profRef(),p,{merge:true});alert(T('Guardado.'));}
function bindForms(){
  ...
  $('payrollForm').onsubmit=e=>{e.preventDefault();const t=teamBy($('prTeam').value);if(!t.id)return alert('Selecciona empleado/equipo.');const gross=Number($('prGross').value||0),bonus=Number($('prBonus')?.value||0),ded=Number($('prDeductions').value||0),adv=Number($('prAdvance')?.value||0),net=Math.max(0,gross+bonus-ded-adv);add('payroll',{teamId:t.id,teamName:t.name,date:$('prDate').value,period:$('prPeriod').value,hours:Number($('prHours')?.value||0),overtime:Number($('prOvertime')?.value||0),gross,bonus,advance:adv,deductions:ded,net,method:$('prMethod').value,note:$('prNote').value});add('cashflow',{date:$('prDate').value,type:'Gasto',concept:`Nómina ${t.name}`,amount:net});e.target.reset();};
