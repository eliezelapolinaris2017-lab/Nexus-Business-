...
async function completeFollowup(id){const f=state.followups.find(x=>x.id===id);if(!f)return;await updateDoc(docPath('followups',id),{status:'Completado',completedAt:today(),updatedAt:serverTimestamp()});if(f.kind==='maintenance' && Number(f.frequencyMonths||0)>0){await add('followups',{...f,status:'Programado',dueDate:plusDays(Number(f.frequencyMonths||6)*30),previousFollowupId:id,completedAt:'',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});}}
...
function bindForms(){
  ...
  if($('followupForm')) $('followupForm').onsubmit=async e=>{e.preventDefault();const c=clientBy($('fClient')?.value||'');if(!c.id)return alert('Selecciona cliente.');const a=assetBy($('fAsset')?.value||'');await add('followups',{kind:$('fKind').value,clientId:c.id,clientName:c.name,assetId:a.id||'',assetName:a.id?assetName(a):'',title:$('fTitle').value,dueDate:$('fDue').value,status:$('fStatus').value,frequencyMonths:$('fKind').value==='maintenance'?6:0,notes:$('fNotes').value});e.target.reset();if($('fDue'))$('fDue').value=plusDays(180);};
