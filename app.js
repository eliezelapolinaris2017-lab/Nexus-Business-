...
function tables(){const i=industry();
  ...
  $('servicesTable').innerHTML=table(['ID','Fecha',i.client,'Activo',i.team,'Detalle','Monto','Comisión','Factura','Acción'],state.services.map(s=>{const t=teamBy(s.teamId),inv=state.invoices.find(x=>x.serviceId===s.id),amount=serviceAmount(s),comm=amount*Number(t.rate||0)/100;return `<tr><td><code>${esc(String(s.id).slice(0,7))}</code></td><td>${esc(s.date)}</td><td>${esc(s.clientName)}</td><td>${esc(s.assetName||'')}</td><td>${esc(s.teamName)}</td><td><b>${esc(serviceTitle(s))}</b><br><span class="muted">${esc((s.fields||[]).filter(Boolean).slice(0,3).join(' · '))}</span></td><td>${money(amount)}</td><td>${money(comm)}</td><td>${inv?esc(inv.number):`<button data-invoice="${s.id}" type="button">Facturar</button>`}</td><td>${action('services',s.id)}</td></tr>`}));
